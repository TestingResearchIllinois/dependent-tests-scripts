...
function compute_selection() {
    ...
    for p in $(ls | grep "_output$"); do
        ...
        if [[ ${found} == 1 ]]; then
            if [[ ${omittedtotaltime} != 0 ]]; then
                ...
                echo "\Def{${projmod}_${testtype}_testtimediff_sele}{0\%}"
            fi
        else
    ...
    if [[ ${rollingfound} == 1 ]]; then
        if [[ ${rollingomittedtotaltime} != 0 ]]; then
            ...
            echo "\Def{overall_${testtype}_testtimediff_sele}{0\%}"
        fi
    else
...
function compute_parallelization_longest() {
    ...
    for p in $(ls | grep "_output$"); do
        ...
        for omitted in $(find ${p} -name PARALLELIZATION-* | grep OMITTED | grep "true"); do
            ...
            if [[ $(echo ${omitted} | grep "${firstsha}") == "" ]]; then
                continue
            fi
...
# Compute differences but not by matching up machine; simply match up the longest-running machine in both orders
function compute_parallelization_longest_no_match() {
    ...
    for p in $(ls | grep "_output$"); do
        ...
        for omitted in $(find ${p} -name PARALLELIZATION-* | grep OMITTED | grep "true"); do
            ...
            # Want to just get times corresponding to longest
            longestomittedtime=0
            longestgiventime=0

            IFS=$'\n'
            # Search for the longest omitted order time
            for ot in $(grep "Execution time for executing the following testing order: " ${omitted}); do
                omittedtesttime=$(echo ${ot} | cut -d' ' -f9)
                # If the omitted time is greater than the longest time, we keep it
                if [[ $(echo "${omittedtesttime} > ${longestomittedtime}" | bc -l) == 1 ]]; then
            ...
            # Search for the longest given order time
            for gt in $(grep "Execution time for executing the following testing order: " ${given}); do
                giventesttime=$(echo ${gt} | cut -d' ' -f9)
                # If the given time is greater than the longest time, we keep it
                if [[ $(echo "${giventesttime} > ${longestgiventime}" | bc -l) == 1 ]]; then
                    longestgiventime=${giventesttime}
                fi
            done
...
function compute_parallelization() {
    ...
    for p in $(ls | grep "_output$"); do
        ...
        # Compare the sum of all the sub test times in the files
        for omitted in $(find ${p} -name PARALLELIZATION-* | grep OMITTED | grep "true"); do
            # Only keep track of immediate next revision for parallelization
            if [[ $(echo ${omitted} | grep "${firstsha}") == "" ]]; then
                continue
...
(
    ...
    # Parallelization
    compute_parallelization_longest_no_match ${testtype}
)
done
