        ...
        for testtype in ORIG AUTO; do
            ...
            for cov in FUNCTION STATEMENT; do
                for type in ABSOLUTE RELATIVE; do
                    ...
                    for f in $(find -name "PRIORITIZATION-${testtype}-*${cov}-${type}*true.txt" | xargs grep -l "Number of DTs fixed: [^0]"); do
            ...
            for type in ORIGINAL TIME; do
                for machine in TWO FOUR EIGHT SIXTEEN; do
                    ...
                    totaldepstime=0
                    for f in $(find -name "PARALLELIZATION-${testtype}-*${type}-*${machine}*true.txt" | xargs grep -l "Number of DTs fixed: [^0]" | head -1); do
                        # Get the total amount of time to get dependencies
            ...
            if [[ ${rollingseledepscount} != 0 ]]; then
                ...
                echo "\\Def{${proj}_$(echo ${testtype} | tr '[:upper:]' '[:lower:]')_deptime_sele_avg}{$(echo "${rollingseledepstime} / ${rollingseledepscount}" | bc -l | cut -d'.' -f1)}"
            else
