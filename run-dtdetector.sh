...
if [[ "$PREFIX" = "old" ]]; then
    cd $DT_SUBJ_SRC
    java -Xmx8000M -cp $DT_TOOLS:$DT_TESTS:$DT_CLASS:$DT_LIBS:$DT_RANDOOP: edu.washington.cs.dt.impact.tools.detectors.DetectorMain --mode RANDOM --rounds 100 --test-order "$DT_SUBJ/${SUBJ_NAME}-${ORDER}-order" --output "$RANDOMIZE_RESULTS"
else
    cd $NEW_DT_SUBJ_SRC
    java -Xmx8000M -cp $DT_TOOLS:$NEW_DT_TESTS:$NEW_DT_CLASS:$NEW_DT_LIBS:$NEW_DT_RANDOOP: edu.washington.cs.dt.impact.tools.detectors.DetectorMain --mode RANDOM --rounds 100 --test-order "$NEW_DT_SUBJ/${SUBJ_NAME}-${ORDER}-order" --output "$RANDOMIZE_RESULTS"
fi
