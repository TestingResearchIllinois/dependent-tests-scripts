...
if [[ "$PREFIX" = "old" ]]; then
    cd $DT_SUBJ_SRC
    java -Xmx8000M -cp $DT_TOOLS: edu.washington.cs.dt.impact.tools.detectors.DetectorMain -cp "$DT_TESTS:$DT_CLASS:$DT_LIBS:$DT_RANDOOP" --mode RANDOM --rounds 100 --test-order "$NEW_DT_SUBJ/${SUBJ_NAME}-${ORDER}-order" --output "$RANDOMIZE_RESULTS"
else
    cd $NEW_DT_SUBJ_SRC
